...
# Platform
To get an account on Verhaert Connect Platform, send a request on `satellite-iot-support` Discord channel and central organizers will give you access.
- Platform: [Verhaert Connect](https://verhaert.allthingstalk.com)  
- Platform Binary Conversion Guide: [AllThingsTalk Binary Conversion Language (ABCL)](https://docs.allthingstalk.com/developers/custom-payload-conversion/)

## Steps
- On Verhaert platform, add a new Arduino device.
- Go to **Device Settings > Connectivity** and choose **Kineis Satellite**.
- Find the Device ID of your Kineis KIM1 module (6 numbers, check example shown below), enter it in the **Kineis Device ID** field and click **Connect to Network**.  
![Diagram](extras/kineisid.jpg)
...
Satellite passings at your location are published by KINEIS and are available in the Extras folder: [Satellite Pass Files - 9th Hackathon 16-18.5.2025.zip](extras/Satellite_Pass_Files_9th%20Hackathon_16-18.5.2025.zip)
- The buzzer on the kit will produce a sound when:
    - KIM1 is not detected: `[1000ms sound] > [500ms pause] > [1000ms sound]`
    - Payload sending fails: `[200ms sound]`
    - Button press is registered: `[50ms sound]`
- The LED on the kit will light up when the KIM1 module is active.
- The range for values of light, sound and potentiometer are from 0 to 255.
